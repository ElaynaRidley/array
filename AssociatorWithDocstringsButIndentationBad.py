...
class Associator:
	...
	def _hash(self, key):
		'''hash takes a string and returns and index number by
			by adding the ords of the first and last letter of the string
			and moding it by the length of the array
		'''
		sum = 0
		if len(key) == 1:
			sum += ord(key)
	...
	def exists(self, key):
			'''this checks to see if a key exists in the primary area. if it does it returns its index. if it doesnt it returns -1'''
	...
	def find(self, value):
			'''this finds a key in the primary area and returns its index if found, -1 if not found'''
		#extra credit
		for i in range(len(self.primaryKeys)):
			if self.primaryKeys[i] == key:
						return i
	...
	def stats(self):
			'''this returns a list ofthe size of the primary area, the number of primary area slots in use, the size of the secondary area, and the number of secondary area slots used'''
	...
	def print(self):
			'''this prints the primary and secondary areas in a table format'''
		print("PRIMARY AREA")
		print("--------------------")
		count=0
		for i in range(len(self.primaryKeys)):
	...
	def _find_ovflow(self, key):
			''' this looks to see if a key exists in the overflow. if it does it returns the index. if it doesnt it returns -1.'''
		for i in range(len(self.overflowKeys)):
			if self.overflowKeys[i] == key:
				return i
		return -1
	...
	def _find_empty(self):
			'''this finds the next empty index in the overflow area'''
		for i in range(len(self.overflowKeys)):
			if self.overflowKeys[i] == None:
				return i

	def clear(self):
			'''this empties out the primary ans secondary area'''
	...
	def dump(self):
			'''this prints out the primary and secondary areas but unformatted'''
		s = ""
